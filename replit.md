...
- **iOS production build + TestFlight submission verified end-to-end (2026-04-28).** Build `a1012e1e-4679-471c-bf1b-c2bce91df570` (production, app build version 6) finished and was submitted via `eas submit --platform ios --profile production --non-interactive` (submission `36ff9811-e372-4a6f-b4eb-cffb287a6737`). App Store Connect confirms build id `6e7ae47d-4f80-42e4-81bb-7a7a8c0ee473` for `三殊胜` (bundle `com.bicwny.sanshusheng`, ASC App ID `6764305709`) with `processingState=VALID`, `internalBuildState=READY_FOR_BETA_TESTING`, `externalBuildState=READY_FOR_BETA_SUBMISSION`. Internal TestFlight testers can install it now; external TestFlight requires submitting for Beta App Review from App Store Connect.
- **App Store Connect API Key (P8) for non-interactive iOS submit:** stored locally as `./.secrets/AuthKey_<KEY_ID>.p8` (gitignored via `*.p8` and `.secrets/` rules) and wired into `submit.production.ios` via the trio `ascApiKeyPath` / `ascApiKeyId` / `ascApiKeyIssuerId`. Because `IosSubmitCommand.js` reads these directly from the submit profile, this bypasses the interactive "Set up an API Key with EAS" flow that otherwise breaks `--non-interactive`. The Key ID and Issuer ID values currently live in `eas.json` — if the repo is ever made public or shared, rotate the ASC key (Users & Access → Integrations in App Store Connect) and re-run the same setup.
