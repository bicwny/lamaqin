...
-- Step 1: Drop all existing policies (if any exist)
DROP POLICY IF EXISTS "Allow authenticated users to read meditation topics" ON public.meditation_topics;
DROP POLICY IF EXISTS "Allow service role to insert meditation topics" ON public.meditation_topics;
...
-- Step 3: Insert the data first (this will work with RLS disabled)
DO $$
DECLARE
...
BEGIN
    ...
    -- Insert all 92 meditation topics
