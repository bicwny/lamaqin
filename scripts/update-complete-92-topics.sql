...
-- Complete update of meditation_topics table with all 92 practices from 前行实修法
-- This script will populate the database with the exact topics from your attached file

DO $$
DECLARE
    meditation_practice_id uuid;
BEGIN
    -- Find or create the 前行观修 practice
    ...
    IF meditation_practice_id IS NULL THEN
        ...
        VALUES ('前行观修', 'time', '座', '前行实修法92座观修系统')
        RETURNING id INTO meditation_practice_id;
        
        RAISE NOTICE '创建了新的前行观修practice，ID: %', meditation_practice_id;
    ELSE
        RAISE NOTICE '找到现有的前行观修practice，ID: %', meditation_practice_id;
    END IF;

    -- Clear existing topics for this practice
    DELETE FROM meditation_topics WHERE practice_id = meditation_practice_id;
    
    RAISE NOTICE '已清除现有的观修主题';

    -- Insert all 92 meditation topics with exact titles from your file
    ...
    RAISE NOTICE '✅ 已成功插入92个前行实修法观修主题！';
    ...
    BEGIN
        ...
        RAISE NOTICE '📊 验证：共插入了 % 个观修主题', topic_count;
    END;
END $$;

-- Also create the meditation_topics table if it doesn't exist
...
-- Enable RLS for meditation_topics
ALTER TABLE meditation_topics ENABLE ROW LEVEL SECURITY;

-- Allow read access to meditation topics
CREATE POLICY IF NOT EXISTS "Allow read access to meditation topics" 
ON meditation_topics FOR SELECT USING (true);
