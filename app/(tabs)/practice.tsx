  ...
  return (
    ...
    >
      ...
      {projects.map((project) => {
        ...
        const practiceDisplayType = project.target_end_date
          ? "固定时长"
          : "持续进行";
          ...
          <View key={project.id} style={styles.practiceCard}>
                ...
                <View style={styles.projectNamePill}>
                  <Text style={styles.projectNameText}>
                    {project.project_name ||
                      presetProjectNames[project.preset_project_id] ||
                      "预设项目"}
                  </Text>
                </View>
              )}
            ...
            <View style={styles.progressContainer}>
              {project.practices.type === "count" ? (
                ...
                <View>
                  {project.target_period === "weekly" ? (
                    <Text style={styles.practiceInfo}>
                      本周目标：{project.daily_target}座 (每周
                      {project.daily_target}座)
                    </Text>
                  ) : (
                    <Text style={styles.practiceInfo}>
                  ...
                  <WeeklyProgressDisplay project={project} user={user} />
                </View>
              )}
            </View>
            ...
            <View style={styles.buttonRow}>
              <TouchableOpacity
                ...
                onPress={() =>
                  handleViewDetails(project.id, project.practices.name)
                }
              >
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  handleCustomRecord(project.id, project.practices.name)
                }
              >
...
function WeeklyProgressDisplay({
  project,
  user,
}: {
  ...
  const todayDetails = todayRecords
    .map((record, index) => `第${index + 1}座${record.duration_minutes}分钟`)
    .join("；");
  ...
  if (isWeekly) {
    ...
    return (
      <View style={styles.weeklyProgress}>
        ...
        {todayCount > 0 && todayDetails && (
          <Text style={styles.weeklyProgressSubtext}>今日：{todayDetails}</Text>
        )}
      </View>
    );
  } else {
    return (
      <View style={styles.weeklyProgress}>
        ...
        {todayCount > 0 && todayDetails && (
          <Text style={styles.weeklyProgressSubtext}>{todayDetails}</Text>
        )}
      </View>
    );
  }
